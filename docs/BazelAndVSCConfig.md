...
В **debug** сборке можно заметить поле **"setupCommands"**, в котором указан **-enable-pretty-printing** - параментр отладчика `gdb`, который позволяет красиво выводить значения **STL**-контейнеров: (фотка)
